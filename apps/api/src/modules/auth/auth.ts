...
export async function issueToken(user: { _id: unknown; role: string; email: string }): Promise<string> { return new SignJWT({ role: user.role, email: user.email }).setProtectedHeader({ alg: "HS256" }).setIssuer(env.JWT_ISSUER).setAudience(env.JWT_AUDIENCE).setSubject(String(user._id)).setIssuedAt().setExpirationTime(env.ACCESS_TOKEN_TTL).sign(secret); }
